...
class GPT(nn.Module):
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear): # initialize linear layers
            ...
            torch.nn.init.normal_(module.weight, mean=0.0, std=std) # normal distribution with mean 0 and std 0.02
...
if ddp:
    ...
    ddp_rank = int(os.environ['RANK']) # global rank, e.g. - if we have 2 machines with 4 GPUs each, the global rank will be 0-7, first machine will have 0-3, second machine will have 4-7
    ddp_local_rank = int(os.environ['LOCAL_RANK']) # single node with multiple GPUs, local rank is the GPU number. e.g. 0-3 are the GPUs on the first machine (local)
...
# To reach the total_batch_size of 524,288 tokens, the model accumulates gradients over multiple passes. 
# For W = 8 (8 GPUs): grad_accum_steps = 1
# For W = 4 (4 GPUs): grad_accum_steps = 2
# For W = 2 (2 GPUs): grad_accum_steps = 4
# For W = 1 (1 GPU): grad_accum_steps = 8
...
# The optimizer updates the model's parameters using gradients (computed through backpropagation) to reduce the loss.
# The learning rate (LR) determines the step size of these updates.
...
# Cosine decay phase - Starts at max_lr. Gradually decreases following a cosine curve until it reaches min_lr at the end of training.
# If the total steps (it) exceed max_steps, the learning rate is clamped to min_lr.
def get_lr(it):
...
# Multiple GPU tensor cores perform operations at great speed (tera flops), but they are contraint by the memory bandwidth (speed at which the the bits are read from the memory). 
# Because the data needs to be moved to the tensor cores, and if the memory bandwidth is slow, the tensor cores will be idle. In nutshell, even 60% of usage of tensor cores is great.
# https://developer.nvidia.com/blog/programming-tensor-cores-cuda-9/
# All our main operations are matrix multiplications, and tensor cores are great at matrix multiplications.
# All linear layers are matrix multiplications, where GELU, LayerNorm, softmax are not very deep operations. 
# Also if you see the biggest matrix multiplication is the 768 (embedding size) to 50257(vocab size) conversion at the top.
...
for step in range(max_steps):
    ...
    # training loop
    '''
    Only read if needed, applicable for micro step,
    1. optimizer.zero_grad(): At the beginning of the training loop, this call resets all gradients of the model parameters to zero. 
    This is important so that gradients from previous training iterations do not interfere with the current accumulation.
    2. loss.backward(): In each micro-step of the loop, after computing the loss (and scaling it down by dividing by grad_accum_steps), you call loss.backward(). 
    This function computes the gradients of the loss with respect to the model parameters and adds (accumulates) these gradients into the .grad attribute of each parameter.
    3. Accumulation Over Micro-Steps: Because you are not calling optimizer.zero_grad() inside the loop, the gradients computed during each micro-step are added to the existing gradients. 
    This way, after several micro-steps (determined by grad_accum_steps), you have effectively accumulated the gradients over a larger “virtual” batch.
    '''
